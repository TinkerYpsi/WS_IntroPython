...
class Player:
    ...
    def play_move(self, opponent):
        rand_num = random.randint(0, len(self.moves) - 1)
        current_move = self.moves[rand_num]
        current_strength = self.move_strengths[rand_num]

        bonus = 20
        die_roll = random.randint(1, 20)
        if die_roll > 5:
            self.move_strengths[rand_num] += bonus      # make winner's move more powerful
...
vader_health_list = [vader.health]              # vader_health_list will be a list of his health after every move
luke_health_list = [luke.health]                # luke_health_list will be a list of his health after every move
